...
export default function FinalScreen({ team, completed, totalXP, onReset }) {
  ...
  function getRank() {
    ...
    if (percent >= 50) return { label: 'Fortgeschritten 🥈', color: 'text-cyan-300' };
    return { label: 'Einsteiger 🥉', color: 'text-teal-300' };
  }

  const rank = getRank();

  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-teal-800 to-cyan-700 flex flex-col items-center justify-center p-6 text-white">
      <div className="bounce-in text-center">
        <div className="text-7xl mb-4">⚓</div>
        <h1 className="text-4xl font-black mb-1">Anker geworfen!</h1>
        <p className="text-cyan-200 text-lg mb-8">Ihr habt euren Weg gefunden.</p>
      </div>
      ...
      <div className="bg-white/95 backdrop-blur text-slate-800 rounded-3xl shadow-2xl p-6 w-full max-w-sm">
        ...
        <div className="bg-cyan-50 rounded-2xl p-4 mb-4 text-center">
          ...
          <p className="text-5xl font-black text-teal-600 mt-1">{totalXP} XP</p>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-teal-50 rounded-2xl p-3 text-center">
            <p className="text-2xl font-black text-teal-600">{completedCount}</p>
        ...
        <button
          onClick={onReset}
          className="w-full bg-teal-600 text-white font-black text-xl py-4 rounded-2xl shadow-lg active:scale-95 transition-transform"
        >
          Nochmal spielen 🌊
        </button>
      </div>
