...
export default function GameScreen({ team, completed, timeLeft, xpPopups, onComplete, onReset, totalXP }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-8">
      {/* Header */}
      <div className="bg-gradient-to-r from-cyan-600 to-teal-700 text-white px-4 pt-6 pb-4 sticky top-0 z-10 shadow-lg">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <span className="text-3xl">{team.icon}</span>
            <div>
              <p className="font-black text-lg leading-tight">{team.name}</p>
              <p className="text-cyan-100 text-sm font-bold">{totalXP} XP</p>
            </div>
          </div>
        ...
        {/* Progress bar */}
        <div className="bg-teal-900/50 rounded-full h-3 overflow-hidden">
          <div
            className="bg-cyan-300 h-3 rounded-full transition-all duration-500"
            style={{ width: `${progress}%` }}
          />
        </div>
        <div className="flex justify-between text-xs text-cyan-100 mt-1">
      ...
      <div className="fixed top-24 left-1/2 -translate-x-1/2 z-50 pointer-events-none">
        {xpPopups.map(p => (
          <div key={p.id} className="xp-pop text-center font-black text-3xl text-cyan-400 drop-shadow-lg">
            +{p.points} XP 🌊
          </div>
        ))}
      </div>
      ...
      <div className="px-4 mt-6">
        <button
          onClick={onReset}
          className="w-full border-2 border-teal-200 text-teal-500 font-bold py-3 rounded-2xl text-sm active:scale-95 transition-transform"
        >
          Spiel zurücksetzen
        </button>
      </div>
