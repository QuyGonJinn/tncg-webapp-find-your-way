...
export default function ControlPage() {
  ...
  useEffect(() => {
    ...
    loadStats();
    pollIntervalRef.current = setInterval(loadStats, 2000);
    ...
    <div className="min-h-screen bg-blue-50 pb-10">
      ...
      <div className={`${isLowTime ? 'bg-gradient-to-r from-red-700 to-red-600' : 'bg-gradient-to-r from-blue-800 to-blue-600'} text-white px-4 pt-6 pb-4 shadow-lg sticky top-0 z-10 transition-colors`}>
        <div className="flex items-center justify-between mb-3">
          <h1 className="text-2xl font-black">📊 Control Dashboard</h1>
          <div className="flex items-center gap-3">
            <div className="text-right">
              <div className={`text-3xl font-black tracking-wider ${isLowTime ? 'animate-pulse' : ''}`}>
                {formatTime(timeLeft)}
              </div>
        ...
        <p className="text-blue-200 text-xs">Aktualisiert: {new Date(stats.timestamp).toLocaleTimeString('de-DE')}</p>
      </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 text-left">
                      ...
                      <div className="bg-blue-200 rounded-full h-2 mt-2 overflow-hidden">
                        <div className="bg-blue-600 h-2 rounded-full transition-all duration-300" style={{ width: `${team.progress}%` }} />
                      </div>
                    </div>
                    ...
                    <div className="ml-1 shrink-0">
                      {team.completed === 12 ? '✅' : team.pending > 0 ? '⏳' : '🎮'}
                    </div>
                    <span className={`text-sm transition-transform shrink-0 ${expandedTeam === team.id ? 'rotate-180' : ''}`}>▼</span>
                  </button>

                  {expandedTeam === team.id && (
                    <div className="p-4 bg-white border-t border-blue-100 space-y-4">
                      {/* PIN + Aktive Stationen */}
                      <div className="grid grid-cols-2 gap-3 bg-blue-50 p-3 rounded-xl">
                        <div>
                          <p className="text-xs text-gray-500 font-bold">Zugangscode</p>
                          <p className="text-xl font-black text-blue-600 tracking-widest">{team.pin}</p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500 font-bold">Aktive Stationen</p>
                          <p className="text-xl font-black text-orange-600">⚡ {team.activeStations}</p>
                        </div>
                      </div>

                      {/* Teilnehmer */}
                      <div>
                        <h3 className="font-bold text-slate-800 mb-2">👥 Teilnehmer</h3>
                        <ParticipantManager
                          teamId={team.id}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-3xl font-black mt-1">
                {overall.totalTeams > 0 ? Math.round((overall.teamsFinished / overall.totalTeams) * 100) : 0}%
              </p>
            </div>
            <div>
