...
export default function HeiligeBuchstabenjagdModerator() {
  const { t } = useI18n();
  ...
  async function loadSubmissions() {
    try {
      const apiBase = `${import.meta.env.VITE_API_URL ?? 'http://localhost:3001'}/api`;
      const response = await fetch(`${apiBase}/heilige-buchstabenjagd/submissions`);
  ...
  async function handleConfirm() {
    ...
    try {
      const apiBase = `${import.meta.env.VITE_API_URL ?? 'http://localhost:3001'}/api`;
      const response = await fetch(`${apiBase}/heilige-buchstabenjagd/submissions/${selectedSubmission.id}/confirm`, {
        method: 'POST',
  ...
  async function handleReject() {
    ...
    if (!confirm(t('heiligeBuchstabenjagd.confirmReject'))) return;

    try {
      const apiBase = `${import.meta.env.VITE_API_URL ?? 'http://localhost:3001'}/api`;
      const response = await fetch(`${apiBase}/heilige-buchstabenjagd/submissions/${selectedSubmission.id}/reject`, {
        method: 'POST',
      });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-purple-100 border-2 border-purple-300 rounded-2xl p-4 text-center">
          <p className="text-purple-900 font-bold text-sm">Ausstehend</p>
          <p className="text-4xl font-black text-purple-700">{pendingCount}</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <button
                ...
                className={`w-full text-left p-4 rounded-xl border-2 transition-all ${
                  selectedSubmission?.id === submission.id
                    ...
                    : 'bg-white border-purple-200 text-stone-800 hover:border-purple-400'
                }`}
              >
          ...
          {/* Detail View */}
          {selectedSubmission && (
            <div className="lg:col-span-2 bg-white rounded-2xl shadow-lg p-6 border-2 border-purple-200">
              <div className="mb-6">
                <h3 className="text-xl font-black text-purple-900 mb-2">{selectedSubmission.team_name}</h3>
                ...
                <div className="mb-6">
                  <img
                    ...
                    alt="Alphabet sheet"
                    className="w-full rounded-xl border-2 border-purple-200 max-h-64 object-cover"
                    onError={(e) => {
                ...
                {selectedSubmission.status === 'pending' && (
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="bg-purple-50 border-2 border-purple-300 rounded-xl px-4 py-3 font-bold text-lg">
