...
export default function AnchorOfHopeModerator() {
  const { t } = useI18n();
  ...
  // Load station code for Anchor of Hope (station 5)
  useEffect(() => {
    loadStationCode();
  }, []);
  ...
  async function loadStationCode() {
    try {
      ...
      // Station 5 is Anchor of Hope
      if (codes[5]) {
        setStationCode(codes[5]);
      }
    } catch (err) {
  ...
  async function loadSubmissions() {
    try {
      const apiBase = `${import.meta.env.VITE_API_URL ?? 'http://localhost:3001'}/api`;
      const response = await fetch(`${apiBase}/anchor-of-hope/submissions`);
  ...
  async function handleConfirm() {
    ...
    try {
      const apiBase = `${import.meta.env.VITE_API_URL ?? 'http://localhost:3001'}/api`;
      const response = await fetch(`${apiBase}/anchor-of-hope/submissions/${selectedSubmission.id}/confirm`, {
        method: 'POST',
  ...
  async function handleReject() {
    ...
    if (!confirm('Möchtest du diese Einreichung wirklich ablehnen?')) return;

    try {
      const apiBase = `${import.meta.env.VITE_API_URL ?? 'http://localhost:3001'}/api`;
      const response = await fetch(`${apiBase}/anchor-of-hope/submissions/${selectedSubmission.id}/reject`, {
        method: 'POST',
      });
  ...
  async function handleDelete() {
    ...
    try {
      const apiBase = `${import.meta.env.VITE_API_URL ?? 'http://localhost:3001'}/api`;
      const response = await fetch(`${apiBase}/anchor-of-hope/submissions/${selectedSubmission.id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-black text-blue-900">⚓ {t('anchorOfHope.title')}</h2>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-blue-100 border-2 border-blue-300 rounded-2xl p-4 text-center">
          <p className="text-blue-900 font-bold text-sm">Ausstehend</p>
          <p className="text-4xl font-black text-blue-700">{pendingCount}</p>
        </div>
        <div className="bg-green-100 border-2 border-green-300 rounded-2xl p-4 text-center">
          <p className="text-green-900 font-bold text-sm">Bestätigt</p>
        ...
        <div className="bg-stone-100 border-2 border-stone-300 rounded-2xl p-4 text-center">
          <p className="text-stone-900 font-bold text-sm">Gesamt</p>
      ...
      {loading ? (
        ...
        <p className="text-center text-gray-500">Keine Einreichungen vorhanden</p>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <button
                ...
                className={`w-full text-left p-4 rounded-xl border-2 transition-all ${
                  selectedSubmission?.id === submission.id
                    ...
                    : 'bg-white border-blue-200 text-stone-800 hover:border-blue-400'
                }`}
              >
          ...
          {/* Detail View */}
          {selectedSubmission && (
            <div className="lg:col-span-2 bg-white rounded-2xl shadow-lg p-6 border-2 border-blue-200">
              <div className="mb-6">
                <h3 className="text-xl font-black text-blue-900 mb-2">{selectedSubmission.team_name}</h3>
                ...
                <div className="mb-6">
                  <img
                    ...
                    alt="Armband"
                    className="w-full rounded-xl border-2 border-blue-200 max-h-64 object-cover"
                    onError={(e) => {
                ...
                {/* Status */}
                <div className="mb-6">
                  <p className="text-sm font-bold text-stone-600 mb-2">Status:</p>
                ...
                {selectedSubmission.status === 'pending' && (
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="bg-blue-50 border-2 border-blue-300 rounded-xl px-4 py-3 font-bold text-lg">
                    ...
                    <div className="flex gap-3">
                      ...
                      >
                        {confirming ? '⏳ ' + t('common.loading') : '✅ Bestätigen'}
                      </button>
                      <button
