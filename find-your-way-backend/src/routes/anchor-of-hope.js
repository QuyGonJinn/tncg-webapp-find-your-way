...
const uploadDir = path.join(__dirname, '..', '..', 'data', 'anchor-of-hope');
if (!fs.existsSync(uploadDir)) {
  fs.mkdirSync(uploadDir, { recursive: true });
  console.log('✅ Created anchor-of-hope upload directory:', uploadDir);
}
...
router.post('/upload', upload.single('photo'), (req, res) => {
  try {
    ...
    const photoPath = `anchor-of-hope/${req.file.filename}`;
    const now = Math.floor(Date.now() / 1000);

    run(
      `INSERT INTO anchor_of_hope_submissions 
...
router.get('/submissions', (req, res) => {
  try {
    const submissions = all(
      `SELECT * FROM anchor_of_hope_submissions ORDER BY submitted_at DESC`
    );
...
router.post('/submissions/:id/confirm', (req, res) => {
  try {
    ...
    const submission = get('SELECT * FROM anchor_of_hope_submissions WHERE id = ?', [id]);
    if (!submission) {
    ...
    const now = Math.floor(Date.now() / 1000);
    run(
      `UPDATE anchor_of_hope_submissions SET status = ?, code = ?, confirmed_at = ? WHERE id = ?`,
      ['confirmed', code, now, id]
    );

    // Broadcast WebSocket event
    broadcast('anchorOfHope:confirmed', {
      submissionId: id,
      teamId: submission.team_id,
      code: code,
      message: `Anchor of Hope submission confirmed for team ${submission.team_name}`,
    });
...
router.post('/submissions/:id/reject', (req, res) => {
  try {
    ...
    const now = Math.floor(Date.now() / 1000);
    run(
      `UPDATE anchor_of_hope_submissions SET status = ?, rejected_at = ? WHERE id = ?`,
      ['rejected', now, id]
    );

    // Broadcast WebSocket event
    broadcast('anchorOfHope:rejected', {
      submissionId: id,
      teamId: submission.team_id,
      message: `Anchor of Hope submission rejected for team ${submission.team_name}`,
    });
...
router.delete('/submissions/:id', (req, res) => {
  try {
    ...
    run(`DELETE FROM anchor_of_hope_submissions WHERE id = ?`, [id]);
...
router.delete('/submissions', (req, res) => {
  try {
    ...
    const submissions = all('SELECT * FROM anchor_of_hope_submissions');
    ...
    run('DELETE FROM anchor_of_hope_submissions');
